...
binom_loglik <- function(p, n_success, n_failure) {
  return(n_success * log(p) + n_failure * log(1 - 
                                                p))
}
...
theta_mle <- n_success/n
eps <- 1e-06
mult = 3

se_theta <- sqrt(theta_mle * (1 - theta_mle)/n)
theta_range <- theta_mle + c(-1, 1) * mult * 
  se_theta
...
loglik <- binom_loglik
alg_score <- binom_alg_score
alg_obs_info <- binom_alg_obs_info
...
# plot
#========
x = seq(theta_range[1], theta_range[2], length.out = 100)
y = loglik(x, n_success = n_success, n_failure = n_failure)
...
plot(x, y,
     ...
     xlab = TeX("$\\theta$"),
     ylab = TeX("$\\log{L(\\theta)}$"),
     main = "Testul lui Wilks (LRT) si a lui Wald ")

# vertical lines
segments(x0 = theta0, 
         y0 = min(y)-0.5,
         x1 = theta0,
         y1 = loglik(theta0, n_success = n_success, n_failure = n_failure),
         lty = 2,
         col = "grey80")

segments(x0 = theta_mle, 
         y0 = min(y)-0.5,
         x1 = theta_mle,
         y1 = loglik(theta_mle, n_success = n_success, n_failure = n_failure),
         lty = 2,
         col = "grey80")

text(x = (theta0+theta_mle)/2,
     y = min(y) + 0.65,
     label = "Wald")

# horizontal lines
segments(x0 = theta0, 
         y0 = loglik(theta0, n_success = n_success, n_failure = n_failure),
         x1 = theta_range[1]-0.5,
         y1 = loglik(theta0, n_success = n_success, n_failure = n_failure),
         lty = 2,
         col = "grey80")

segments(x0 = theta_mle, 
         y0 = loglik(theta_mle, n_success = n_success, n_failure = n_failure),
         x1 = theta_range[1]-0.5,
         y1 = loglik(theta_mle, n_success = n_success, n_failure = n_failure),
         lty = 2,
         col = "grey80")

text(x = theta_range[1]+0.03,
     y = (loglik(theta0, n_success = n_success, n_failure = n_failure)+loglik(theta_mle, n_success = n_success, n_failure = n_failure))/2,
     label = "Wilks")
...
axis(side = 1, # bottom
     cex.axis = 0.8)
axis(side = 1, # bottom
     at = c(theta0, theta_mle),
     labels = c(TeX("$\\theta_0$"), TeX("$\\hat{\\theta}$")),
     cex.axis = 0.6,
     # tick = FALSE,
     line = -0.8)
...
axis(side = 2, # left
     las = 1, 
     at = seq(-24, -14, by = 2),
     labels = c(seq(-24, -16, by = 2), ""),
     cex.axis = 0.8)

axis(side = 2, # left
     las = 1,
     at = c(loglik(theta0, n_success = n_success, n_failure = n_failure), 
            loglik(theta_mle, n_success = n_success, n_failure = n_failure)),
     labels = c(TeX("$\\log{L(\\theta_0)}$"), TeX("$\\log{L(\\hat{\\theta})}$")),
     cex.axis = 0.6,
     # tick = FALSE,
     line = 0)
...
```{r, echo=FALSE}
tab_H1 = data.frame(theta_hat = theta_hat)

rownames(tab_H1) = c("beta", "rho")

if (getOutputFormat() == "pdf_document"){
  kable(tab_H1, format = "latex", longtable = T, booktabs = T,
        col.names = c("MLE")) %>%
    kable_styling(latex_options = c("striped", "repeat_header"))
}else{
  kable(tab_H1, booktabs = T,
        col.names = c("MLE")) %>%
    kable_styling(bootstrap_options = c("striped", "hover"))
}
...
iar logaritmul funcției de verosimilitate este $l(y|x;\hat{\boldsymbol{\theta}}) =$ `r round(loglik_theta, 4)`.
...
prin urmare $l(y|x;\hat{\boldsymbol{\theta}}_{H_0}) =$ `r round(loglik_theta_H0, 4)`

```{r, echo=FALSE}
tab_H0 = data.frame(theta_hat = theta_hat_H0)

rownames(tab_H0) = c("beta")

if (getOutputFormat() == "pdf_document"){
  kable(tab_H0, format = "latex", longtable = T, booktabs = T,
        col.names = c("MLE")) %>%
    kable_styling(latex_options = c("striped", "repeat_header"))
}else{
  kable(tab_H0, booktabs = T,
        col.names = c("MLE")) %>%
    kable_styling(bootstrap_options = c("striped", "hover"))
}
