...
```{r, echo = FALSE, out.width=dimplot2}
plot(palindromes$location,
     rep(0.25, nrow(palindromes)), 
     col = "grey10",
     pch = "I",
     yaxt = "n", 
     ylab = "", 
     main = "Locatia palindroamelor",
     xlab = "",
     bty = "n")
```
...
Observăm că în primul segment ADN de lungime 4000 avem `r counts[1]` palindroame iar în al doilea segment avem `r counts[2]`. Repartiția numărului de palindroame este 

```{r, echo = FALSE, out.width=dimplot2}
barplot(table(counts),
        main = "Numarul de palindroame in intervale de 4000 bp",
        # cex.main = 0.8, 
        col = "grey80")
```

Din figura de mai sus observăm că din cele `r length(counts)` de segmente de ADN de 4000 de perechi de bază, `r sum(counts %in% c(1,2))` conțin cel mult 2 palindroame, `r sum(counts == 3)` conțin 3 palindroame iar `r sum(counts == 4)` conțin 4 palindroame. De asemenea `r sum(counts >= 9)` conțin cel puțin 9 palindroame. Aceste segmente acoperă primele `r format(intervals[length(intervals)], 6)` perechi de bază, ultimele `r 229354 - intervals[length(intervals)]` fiind excluse din construcția intervalelor (acestea mai conțin `r sum(loc>intervals[length(intervals)])` palindroame). Prin urmare considerăm doar `r length(loc) - sum(loc>intervals[length(intervals)])` palindroame. 
...
```{r, echo = FALSE, out.width=dimplot2}
plot(table(counts)/sum(table(counts)),
     lwd = 6, 
     col = "grey80", 
     main = "Repartitia efectivelor observate",
     xlab = "Numar de palindroame in 4000 bp", 
     ylab = "",
     bty = "n",
     # cex.main = 0.9,
     cex.lab = 0.8)

lines(1:14 - 0.25, dpois(1:14, lambda),
      type = "h",
      col = "brown3",
      lwd = 6)

legend("topright", 
       legend = c("observate", paste0("Poisson(",round(lambda, 3),")")),
       fill = c("grey80", "brown3"), 
       cex = 0.7,
       bty = "n")
```
...
fcol = c("<=2", paste(3:8), ">=9")

tab.pal = data.frame(interval = fcol,
                     observed = observed,
                     expected = expected)


if (getOutputFormat() == "pdf_document"){
  kable(tab.pal, format = "latex", longtable = T, booktabs = T,
      align = "ccc",
      col.names = c("Interval", "Efective observate", "Efective asteptate")) %>%
    kable_styling(latex_options = c("striped", "repeat_header"))
}else{
  kable(tab.pal, booktabs = T,
      align = "ccc",
      col.names = c("Interval", "Efective observate", "Efective asteptate")) %>%
    kable_styling(bootstrap_options = c("striped", "hover"))
}
...
iar statistica de test devine $X^2=$ `r sum((observed - expected)^2/expected)`. 
...
și prin urmare nu putem respinge ipoteza nulă prin care repartiția numărului de palindroame pe intervale de lungime 4000 de perechi de bază este Poisson de medie $\lambda=$ `r round(lambda, 3)`.
