...
$$
  ...
  1. Notăm cu $y = \hat{\beta_0} + \hat{\beta_1} x$ dreapta de regresie. Calculați coeficienții $\hat{\beta}_0$ și $\hat{\beta}_1$.
...
f2 = function(x1, x2){
  return(x1 + 3*x2 + 2*x1*x2)
}

f3 = function(x1, x2){
  return(3 + x1 + 8*x2 - 4*x1*x2 + 2*x1^2 + 3*x2^2)
}

par(mfrow = c(1, 3),
    oma = c(5, 0, 5, 0), # two rows of text at the outer left and bottom margin
    mar = c(0, 0, 0, 0)+0.1, # space for one row of text at ticks and to separate plots
...
persp(x1, x2, z1, 
      ...
      zlab = "f1",
      shade = 0.5,
      main = TeX("$f_1(x_1,x_2) = -1+2x_1+3x_2$"),
      cex.main = 0.95)
...
persp(x1, x2, z2,
      ...
      zlab = "f2",
      shade = 0.5,
      main = TeX("$f_2(x_1,x_2) = x_1 + 3x_2 + 2x_1x_2$"),
      cex.main = 0.95)
...
persp(x1, x2, z3,
      ...
      zlab = "f3",
      shade = 0.5,
      main = TeX("$f_3(x_1,x_2) = 3 + x_1 + 8x_2 - 4x_1x_2 + 2x_1^2 + 3x_2^2$"),
      cex.main = 0.95)
...
```{r Gauss-Markov, type = "rmdexercise", engine = engine_block}
...
if (getOutputFormat() == "pdf_document"){
  knitr::include_graphics("images/sem2/RegMult_p2b.pdf")
}else{
  knitr::include_graphics("images/sem2/RegMult_p2b.png")
}
...
if (getOutputFormat() == "pdf_document"){
  knitr::include_graphics("images/sem2/RegMult_p2c.pdf")
}else{
  knitr::include_graphics("images/sem2/RegMult_p2c.png")
}
...
par(mfrow = c(1,4),
    oma = c(5, 0, 5, 0), # two rows of text at the outer left and bottom margin
    mar = c(0, 0, 0, 0)+0.05, # space for one row of text at ticks and to separate plots
    mgp = c(0, 0, 0) + 0.1,    # axis label at 2 rows distance, tick labels at 1 row
    xpd = NA)            # allow content to protrude into outer margin (and beyond)

label_main = c("$\\alpha = 0$", "$\\alpha = \\frac{\\pi}{4}$", 
               "$\\alpha = \\frac{\\pi}{2}$", "$\\alpha = \\frac{3\\pi}{2}$",
               "$\\alpha = \\pi$")

th = seq(0,3*pi/4,by=pi/4)

for (k in seq_along(th)){
    #rotation matrix
    Rot = matrix(c(cos(th[k]), -sin(th[k]), sin(th[k]), cos(th[k])),2); 
    A = matrix(c(ax1, 0, 0, ax2), 2)
    ...
    prob.z = z
    persp(x, y, prob.z, theta = 30, phi = 30, 
          expand = 0.5,
          col = "lightblue",
          box = FALSE,
          cex.main = 0.7)

    title(TeX(label_main[k]))
}
...
if (getOutputFormat() == "pdf_document"){
  knitr::include_graphics("images/sem2/RegMult_p3.pdf")
}else{
  knitr::include_graphics("images/sem2/RegMult_p3.png")
}
