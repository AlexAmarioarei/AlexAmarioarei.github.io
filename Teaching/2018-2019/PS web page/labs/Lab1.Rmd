...
major_releases = rbind(major_releases, pkgs[which(pkgs$version == "3.5"), ])

#
p = ggplot(pkgs, aes(x = date, y = pkgs)) + 
  geom_smooth(fill = myblue, colour = mybrown) +
  geom_point(size = 2, colour = myred) + 
  # geom_rug(colour = "grey50") +
  geom_vline(data = major_releases, 
             aes(xintercept = as.numeric(date)), 
             colour = "#6C7B8B", linetype = 2) +
  geom_text(data = major_releases, 
            aes(label = paste("Versiunea", version), y = c(8000, 9500, 8000)), 
            angle = 90, 
            colour = "grey20",
            hjust = 1, vjust = -1) +
  geom_text_repel(data = pkgs[which(pkgs$version %in% c("2", "2.5", "2.11", "3", "3.3", "3.5")), ], 
...
knitr::include_graphics("images/Lab1/RStudio3.jpg")
```
...
Table: `r tn("Operatii binare in R")`
...
Table: `r tn("O serie de link-uri utile")`
...
Table: `r tn("Functii utile pentru crearea unui vector")`
...
x = c(106, 123, 123, 111, 125, 113, 130, 113, 114, 100, 120, 130, 118, 114, 127, 112, 121, 114, 120, 119, 127, 114, 108, 127, 131, 157, 102, 133)
zile = rep(c("L", "Ma", "Mi", "J", "V", "S", "D"), 4)
sapt = rep(1:4, each = 7)

cat("Cel mai mult s-a vorbit in zilele", zile[which(x == max(x))], "din saptamana", sapt[which(x == max(x))], "\n")
cat("Cel mai putin s-a vorbit in zilele", zile[which(x == min(x))], "din saptamana", sapt[which(x == min(x))], "\n")
cat("Zilele in care s-a vorbit mai mult de 120 minute sunt", zile[x>=120], "din saptamanile", sapt[x>=120], "\n")
```
