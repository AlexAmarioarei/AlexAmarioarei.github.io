...
showConvergenceProbability = function(N = 2000, m = 10, e = 0.05, n = floor(N/2), 
  ...
  abline(h = 0, col = myred, lty = 2, lwd = 2)
  
  for( j in 1:length(n)){
    pj = sum(y[n[j],]>mu+e | y[n[j],]<mu-e)/m
    
    segments(n[j]-step, a, n[j]-step, b, lty = 2, col = myblue)
    segments(n[j]+step, a, n[j]+step, b, lty = 2, col = myblue)
...
Fie $X_1, X_2, \ldots$ un șir de variabile aleatoare independente și indentic repartizate, de medie $\mathbb{E}[X_1] = \mu<\infty$ și varianță $Var(X_1) = \sigma^2<\infty$. Atunci $\forall \epsilon>0$ avem 
...
xmat = matrix(runif(N*m), ncol = m)
y = apply(xmat, 2, cumsum)/1:N

p = apply(y, 1, function(x){sum(x>mu+e | x<mu-e)/m})

par(mfrow = c(1,2))

# first plot
plot(1:N, y[,1], col = "grey80", type = "l",
     bty = "n", 
     axes = FALSE,
     ylim = c(0.4, 0.6), 
     ylab = "",
...
for (i in 2:m){
  lines(1:N, y[,i], col = "grey80")
}

abline(h = mu - e, lty = 2, lwd = 2)
abline(h = mu + e, lty = 2, lwd = 2)
abline(h = mu, col = myred, lty = 2, lwd = 2)

axis(side = 1, at = c(1, n, N), 
     labels = c("1", TeX(paste0("$", "n_", 1:length(n), "$")), "N"), 
     font = 1)

axis(side = 2, at = c(mu-e, mu, mu+e), 
     labels = c(TeX("$\\mu - \\epsilon$"), TeX("$\\mu$"), TeX("$\\mu + \\epsilon$")),
     las = 2,
     font = 1)

axis(side = 2, at = c(0.6), 
     labels = c(TeX("$\\bar{X}_n$")),
     las = 2,
     font = 2,
     tick = FALSE)
...
for( j in 1:length(n)){
    ...
    segments(n[j]-step, 0.45, n[j]-step, 0.55, lty = 2, col = myblue)
    segments(n[j]+step, 0.45, n[j]+step, 0.55, lty = 2, col = myblue)
    
    text(n[j], 0.555, round(p[n[j]], digits = 2),
         cex = 0.85)
}


# second plot
plot(1:N, p, type = "l", col = myblue,
     bty = "n",
     xlab = "",
     ylab = TeX("$p_n$"),
     axes = FALSE)

axis(side = 1, at = c(1, floor(N/2), N), 
     labels = c("1",  "n", "N"), 
     font = 1)
...
Să presupunem că primim o monedă și ni se spune că aceasta aterizează pe fața cap în $48\%$ din cazuri. Vrem să testăm această afirmație. Folosind *Legea numerelor mari* și știind că vrem să fim siguri în $95\%$ din cazuri, ne întrebăm de câte ori trebuie să aruncăm moneda pentru a verifica afirmația?
...
```{r, fig.align='center', out.width=dimplot}
n = 10000
...
Obținem că valoarea numerică a lui $I$ este `r format(I, digits = 4)` iar cea obținută cu ajutorul metodei Monte Carlo este `r format(I2, digits = 4)`. 
...
```{r, type = "rmdinsight", engine = engine_block}
Fie $X_1, X_2, \ldots$ un șir de variabile aleatoare independente și indentic repartizate, de medie $\mathbb{E}[X_1] = \mu<\infty$ și varianță $Var(X_1) = \sigma^2<\infty$. Atunci, notând $S_n = X_1 + \cdots + X_n$, avem
