# Curs Biostatistică 2017 - Laborator 7 & 8
<style type="text/css">
.table {
    margin: auto;
    width: 40%;
...
Pentru un nou set de predictori, $x$, răspunsul prognozat este $\hat{y} = \hat{\beta}_0+\hat{\beta}_1 x$ și vrem să investigăm incertitudinea din această predicție. Putem face distincția între două tipuri de predicție: predicție asupra răspunsului viitor mediu (inferență asupra mediei condiționate $\mathbb{E}[Y|X=x]$) sau predicție asupra observațiilor viitoare (inferență asupra răspunsului condiționat $Y|X=x$). 

Un interval de încredere pentru răspunsul viitor mediu este:

$$
\left(\hat y \pm t_{n-2:\alpha/2}\sqrt{\frac{\hat\sigma^2}{n}\left(1+\frac{(x-\bar x)^2}{s_x^2}\right)}\right)
$$

Un interval de încredere pentru valoarea prezisă (interval de predicție) este:

$$
\left(\hat y \pm t_{n-2:\alpha/2}\sqrt{\hat\sigma^2+\frac{\hat\sigma^2}{n}\left(1+\frac{(x-\bar x)^2}{s_x^2}\right)}\right)
$$
...
<div class="figure" style="text-align: center">
<img src="Lab_7_8_files/figure-html/unnamed-chunk-23-1.png" alt="Nivelul de sare prezis impreuna cu intervalul de incredere de nivel 95% pentru raspunsul mediu" width="90%" />
