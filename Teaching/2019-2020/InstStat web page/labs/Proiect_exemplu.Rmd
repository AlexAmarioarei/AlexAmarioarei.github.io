...
Obiectivul acestui proiect este de a prezenta câteva metode utile atunci când vrem să verificăm dacă eșantionul provine ditnr-o populație normală.
...
Construiți în R două funcții, `skewness_coef()` și `kurtosis_coef()`, care să permită calculul coeficientului de asimetrie și respectiv a coeficientului de aplatizare pentru un eșantion dat.  
```
...
Deplasarea față de valoarea $0$, atât pentru coeficientul de asimetrie cât și pentru cel de aplatizare indică o deplasare față de repartiția normală. Pentru a decide dacă o anumită deplasare față de $0$ este mare sau mică se poate folosi un studiu de simulare. De exemplu, coeficientul de asimetrie calculat pentru greutatea la naștere a celor `r length(MASS::birthwt$bwt)` de copii din setul de date `birthwt` (din pachetul `MASS`) este `r round(skewness_coef(MASS::birthwt$bwt), 3)` iar coeficientul de aplatizare este `r round(kurtosis_coef(MASS::birthwt$bwt), 3)`. Pentru a vedea dacă `r round(skewness_coef(MASS::birthwt$bwt), 3)` și respectiv dacă `r round(kurtosis_coef(MASS::birthwt$bwt), 3)` sunt valori tipice pentru coeficientul de asimetrie și respectiv de aplatizare dintr-un eșantion de `r length(MASS::birthwt$bwt)` de observații dintr-o populație normală, repetăm următorul proces de un număr mare de ori (de exemplu $10000$): generăm aletor `r length(MASS::birthwt$bwt)` de observații dintr-o repartiție normală și calculăm cei doi coeficienți. 
...
Figurile de mai jos reprezintă histogramele a $10000$ de valori ale coeficientului de asimetrie și respectiv de aplatizare, calculate pentru $10000$ de eșantioane de talie `r length(MASS::birthwt$bwt)` dintr-o repartiție normală standard. 
...
```{r, type = "rmdexercise", engine = engine_block}
Construiți în R o funcție `qqgraf()` care să traseze graficul cuantilă-cuantilă pentru două eșantioane.
```  
...
```{r, type = "rmdexercise", engine = engine_block}
Construiți în R o funcție `qq_norm()` care să traseze graficul cuantilă-cuantilă (*normal-quantile plot*) pentru a verifica dacă un eșantion provine dintr-o populație normală.
```  
...
```{r, type = "rmdexercise", engine = engine_block}
Considerați setul de date `birthwt` din pachetul `MASS`. Trasați histograma greutății la naștere a copiilor (variabila `bwt`) și graficul cuantilă-cuantilă pentru o populație normală. Comparați grafic dacă repartiția greutății copiilor la naștere diferă în funcție de statutul de fumător al mamei. 
```  
...
```{r, type = "rmdexercise", engine = engine_block}
Construiți în R o funcție `JBtest()` care să implementeze testul Jarque-Bera. Aplicați testul pentru o serie de exemple. 
```
...
Fie $(X_n)_n$ un șir de variabile aleatoare independente și indentic repartizate cu repartiția comună $\mathbb{P}\circ X^{-1}$ și a cărei funcție de repartiție $F$, este continuă. Atunci șirul funcțiilor de repartiții empirice $(\hat{F}_n)_n$ asociate lui $(X_n)_n$ verifică
...
a1 = round(apply(out_rt3, 1, function(x) mean(x<=0.05, na.rm=TRUE)), 3)
df1 = as.data.frame(t(a1))

# knitr::kable(df1,
#              align = "c",
#       col.names = c("Jarque-Bera", "Shapiro-Wilk", "Shapiro-Francia", "Lilliefors"))

if (getOutputFormat() == "pdf_document"){
  kable(df1, format = "latex", longtable = T, booktabs = T,
        col.names = c("Jarque-Bera", "Shapiro-Wilk", "Shapiro-Francia", "Lilliefors")) %>%
...
a2 = round(apply(out_u, 1, function(x) mean(x<=0.05, na.rm=TRUE)), 3)

df2 = as.data.frame(t(a2))
...
if (getOutputFormat() == "pdf_document"){
  kable(df2, format = "latex", longtable = T, booktabs = T,
        col.names = c("Jarque-Bera", "Shapiro-Wilk", "Shapiro-Francia", "Lilliefors")) %>%
    kable_styling(latex_options = c("striped", "repeat_header"))
}else{
...
a3 = round(apply(out_g22, 1, function(x) mean(x<=0.05, na.rm=TRUE)), 3)

df3 = as.data.frame(t(a3))
...
if (getOutputFormat() == "pdf_document"){
  kable(df3, format = "latex", longtable = T, booktabs = T,
        col.names = c("Jarque-Bera", "Shapiro-Wilk", "Shapiro-Francia", "Lilliefors")) %>%
    kable_styling(latex_options = c("striped", "repeat_header"))
}else{
  kable(df3, booktabs = T,
        col.names = c("Jarque-Bera", "Shapiro-Wilk", "Shapiro-Francia", "Lilliefors")) %>%
    kable_styling(bootstrap_options = c("striped", "hover"))
}
