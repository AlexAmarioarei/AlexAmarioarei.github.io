...
\hypertarget{generarea-unei-variabile-aleatoare-folosind-metoda-respingerii}{%
\section{Generarea unei variabile aleatoare folosind metoda
...
\begin{itemize}
  \item[1.] Folosiți metoda respingerii pentru a genera $10000$ de observații repartizate cu densitatea $f$.
  \item[2.] Trasați histograma acestui eșantion și comparați cu densitatea $f$.
\end{itemize}
\end{rmdexercise}
...
\NormalTok{    x =}\StringTok{ }\KeywordTok{runif}\NormalTok{((}\DecValTok{4}\OperatorTok{*}\NormalTok{m)}\OperatorTok{%/%}\NormalTok{pi }\OperatorTok{+}\StringTok{ }\DecValTok{1}\NormalTok{, }\DecValTok{-1}\NormalTok{, }\DecValTok{1}\NormalTok{)}
...
\KeywordTok{proc.time}\NormalTok{() }\OperatorTok{-}\StringTok{ }\NormalTok{start}
\NormalTok{   user  system elapsed }
   \FloatTok{0.35}    \FloatTok{0.03}    \FloatTok{0.58} 
...
\KeywordTok{proc.time}\NormalTok{() }\OperatorTok{-}\StringTok{ }\NormalTok{start}
\NormalTok{   user  system elapsed }
   \FloatTok{0.38}    \FloatTok{0.03}    \FloatTok{0.62} 
\end{Highlighting}
\end{Shaded}
...
\hypertarget{simularea-unei-uniforme-pe-disc}{%
...
\begin{itemize}
  \item[1.] Aproximați valoarea lui $\pi$ prin ajutorul numărului de puncte $Y_n$ care cad în interiorul discului $D$ (Metoda respingerii)
  
  \item[2.] Simulați $n$ puncte uniforme pe disc. 
\end{itemize}
\end{rmdexercise}
...
generarea a \(n=\) 2000 puncte este 3.156 iar eroarea absoluta este
0.01441.
